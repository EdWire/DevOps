...
if [[ ${#resource_group_name} -gt 64 || ! $resource_group_name =~ ^[a-zA-Z0-9\_-]+$ ]]; then
  echo "Invalid resource_group_name: $resource_group_name"
  echo "Invalid resource_group_name: $resource_group_name" 1>&3
  echo "The chosen resource_group_name must be less than 64 characters, and must contain only letters, numbers, dashes and underscores."
  echo "The chosen resource_group_name must be less than 64 characters, and must contain only letters, numbers, dashes and underscores." 1>&3
  exit 1
fi
